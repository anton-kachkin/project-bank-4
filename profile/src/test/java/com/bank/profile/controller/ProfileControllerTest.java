...
@DisplayName("Тестирование контроллера ProfileController")
class ProfileControllerTest {
    ...
    private List<ProfileDto> dtoList;
    ...
    public void setUp() {
        ...
        dto = dtoFactory.createProfileDto();
        dtoList = List.of(dtoFactory.createProfileDto(),
    ...
    public void testRead() throws Exception {
        ...
        mockMvc.perform(get("/profile/read/{id}", id))
    ...
    void testUpdate() throws Exception {
        when(service.update(eq(id), any(ProfileDto.class))).thenReturn(dto);

        mockMvc.perform(put("/profile/update/{id}", id)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dto)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(id));
    ...
    void testUpdate_Id_NotFound() throws Exception {
        when(service.update(eq(id), any(ProfileDto.class)))
                .thenThrow(new EntityNotFoundException("Entity не найден"));
