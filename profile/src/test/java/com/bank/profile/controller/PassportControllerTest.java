...
@DisplayName("Тестирование контроллера PassportController")
class PassportControllerTest {
    ...
    private List<PassportDto> dtoList;
    ...
    public void setUp() {
        ...
        dto = dtoFactory.createPassportDto();
        dtoList = List.of(dtoFactory.createPassportDto(),
    ...
    public void testRead() throws Exception {
        ...
        mockMvc.perform(get("/passport/read/{id}", id))
    ...
    void testUpdate() throws Exception {
        when(service.update(eq(id), any(PassportDto.class))).thenReturn(dto);

        mockMvc.perform(put("/passport/update/{id}", id)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dto)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(id));
    ...
    void testUpdate_Id_NotFound() throws Exception {
        when(service.update(eq(id), any(PassportDto.class)))
                .thenThrow(new EntityNotFoundException("Entity не найден"));
